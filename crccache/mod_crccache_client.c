...
static int crccache_decode_filter(ap_filter_t *f, apr_bucket_brigade *bb) {
	...
	ap_log_error(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, r->server,
	"cache: running CRCCACHE_DECODE filter");
...
/*            buf = apr_palloc(r->pool, VALIDATION_SIZE);
        ...
        ap_log_error(APLOG_MARK, APLOG_DEBUG, APR_SUCCESS, r->server,
                     "cache: running CRCCACHE_DECODE filter, read %ld bytes",len);

        // TODO: actually decode here
        APR_BUCKET_REMOVE(e);
        APR_BRIGADE_INSERT_TAIL(ctx->bb, e);
...
static const cache_provider crccache_client_provider =
{
    &remove_entity,
    &store_headers,
    &store_body,
...
static void disk_cache_register_hook(apr_pool_t *p)
{
	 ...
	 * his happens.
	 */
	crccache_decode_filter_handle = ap_register_output_filter("CRCCACHE_DECODE",
			crccache_decode_filter, NULL, AP_FTYPE_CONTENT_SET + 1);
}
