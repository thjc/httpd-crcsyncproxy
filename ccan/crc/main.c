...
main(void)
{
	 ...
	 */
	printf("The check value for the %s standard is 0x%"CRC_FORMAT"\n", CRC_NAME, CHECK_VALUE);
	
	/*
	 * Compute the CRC of the test message, slowly.
	 */
	printf("The crcSlow() of \"123456789\" is 0x%"CRC_FORMAT"\n", crcSlow(test, strlen(test)));
	
	/*
	 * Compute the CRC of the test message, more efficiently.
	 */
	crcInit();
	printf("The crcFast() of \"123456789\" is 0x%"CRC_FORMAT"\n", crcFast(test, strlen(test)));
